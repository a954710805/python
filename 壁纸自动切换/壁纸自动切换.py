...
class Bz(object):
    def __init__(self):
        self.curent_url = 'http://pic.tsmp4.net/api/nvsheng/img.php'
        self.root = Tk()
        self.root.attributes("-alpha", 0.8)
        self.root.title('壁纸切换')
        self.root.geometry('218x55+1300+20')
        self.b1 = Button(self.root, text=' 换一类 ', command=self.change_type, fg='#050505',width=8).place(x=0, y=0)
        ...
        self.label = Label(self.root, text='女神',fg='#9e3dff')
    ...
    def run(self):
        ...
        # t1.setDaemon(True)           #设置守护线程 --->是否完
        t1.start()
        self.root.mainloop()
        if self.flag:
            root = Tk()
            root.withdraw()
            messagebox.showinfo("", "关闭进程")
